...
from edutap.wallet_apple.crypto import sign_pkcs7_detached


def test_sign_pkcs7_detached_verifies_against_data(signing_material):
    key, cert, wwdr = create_keys(*signing_material)
    data = b'{"hello":"world"}'
    sig = sign_pkcs7_detached(data, key, cert, wwdr)
